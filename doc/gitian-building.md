...
*Setup instructions for a Gitian build of Ibitcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ibitcoin
...
- [Building Ibitcoin Core](#building-ibitcoin-core)
...
Clone the git repositories for ibitcoin and Gitian.
...
in the ibitcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Ibitcoin Core
----------------

To build Ibitcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ibitcoin repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/ibitcoin-project/ibitcoin
...
For example:
```bash
URL=https://github.com/thrasher-/ibitcoin.git
...
and the ibitcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ibitcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ibitcoin
SIGPATH=/some/root/path/ibitcoin-detached-sigs

./bin/gbuild --url ibitcoin=${BTCPATH},signature=${SIGPATH} ../ibitcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ibitcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ibitcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ibitcoin-osx-build.assert
```
...
[ibitcoin-project/gitian.sigs.ltc](https://github.com/ibitcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
